...
socket.on('connect', function () {
  ...
  socket.emit('loginChat', currentUser, function (usersConnected) {
    ...
    //on chat login, load messages stored into the database
    $.ajax({
      type: 'GET',
...
function renderMessage(message, curUser) {
  ...
  if (message.sender === curUser._id) {
    ...
    if (message.sender === 'Administrator') {
      //sender is the administrator

      class2 = 'box bg-light-danger';
      ...
      let foundUser = users.filter((user) => user._id === message.sender)[0];

      class2 = 'box bg-light-info';
      username = foundUser.username;

      img = `/image/user/${foundUser.img}?token=${foundUser.token}`;
    }
